...
public class sumOfArrays {
    private static int[] sumOfArray(int[] arr1, int[] arr2, int n, int m) {
        int l = Math.max(n, m)+1;
        int[] sumArr = new int[l];
        int sum1 = 0;
        int sum2 = 0;
        for(int i=0; i<n; i++) {
            sum1+=arr1[i]*(int) Math.pow(10, n-i-1);
        }
        for(int i=0; i<m; i++) {
            sum2+=arr2[i]*(int) Math.pow(10, m-i-1);
        }
        int sum3 = sum1+sum2;
        for(int i=l-1; i>=0; i--) {
            sumArr[i] = sum3%10;
    ...
    public static void main(String[] args) {
        ...
        int[] ans = sumOfArray(arr1, arr2, n, m);
